...
export const useDebugGui = ({ 
  ...
  const callbacksRef = useRef({ onHeadRotation, onJawControl, onBoneHelperToggle, onEnvironmentUpdate });
  const settingsRef = useRef(settings);

  // Update refs when callbacks change
  useEffect(() => {
    callbacksRef.current = { onHeadRotation, onJawControl, onBoneHelperToggle, onEnvironmentUpdate };
  ...
  useEffect(() => {
    ...
    const setupGui = async () => {
      try {
        ...
        headFolder.add(settingsRef.current.manualHeadRotation, 'x', -Math.PI, Math.PI).onChange((value) => {
          callbacksRef.current.onHeadRotation({ x: value, y: settingsRef.current.manualHeadRotation.y });
        });
        headFolder.add(settingsRef.current.manualHeadRotation, 'y', -Math.PI, Math.PI).onChange((value) => {
          callbacksRef.current.onHeadRotation({ x: settingsRef.current.manualHeadRotation.x, y: value });
        });
        ...
        jawFolder.add(settingsRef.current.jawControl, 'animationAmplitude', 0, 0.5);
        jawFolder.open();

        // Bone Helpers Folder
        const helpersFolder = gui.addFolder('Bone Helpers');
        helpersFolder.add(settingsRef.current, 'showBoneHelpers').onChange((value) => {
        ...
        envFolder.add(settingsRef.current.environment, 'showBackground').onChange((value) => {
          callbacksRef.current.onEnvironmentUpdate({ ...settingsRef.current.environment, showBackground: value });
        });
        envFolder.add(settingsRef.current.environment, 'backgroundBlur', 0, 1).onChange((value) => {
          callbacksRef.current.onEnvironmentUpdate({ ...settingsRef.current.environment, backgroundBlur: value });
        });
        envFolder.add(
          ...
          callbacksRef.current.onEnvironmentUpdate({ ...settingsRef.current.environment, intensity: value });
        });
        envFolder.add(
          settingsRef.current.environment, 
          'exposure', 
          ...
          callbacksRef.current.onEnvironmentUpdate({ ...settingsRef.current.environment, exposure: value });
        });
        envFolder.add(
          settingsRef.current.environment, 
          'rotation', 
          ...
          callbacksRef.current.onEnvironmentUpdate({ ...settingsRef.current.environment, rotation: value });
        });
        envFolder.open();
      } catch (error) {
