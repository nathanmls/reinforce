...
export default function SceneBackground({
  ...
  useEffect(() => {
    ...
    if (heroProgress < 1) {
      ...
      topColor
        .setHex(COLORS.HERO.TOP)
        .lerp(new THREE.Color(COLORS.WELCOME.TOP), heroProgress);
      bottomColor
        .setHex(COLORS.HERO.BOTTOM)
        .lerp(new THREE.Color(COLORS.WELCOME.BOTTOM), heroProgress);
    } else if (welcomeProgress < 1) {
      // Welcome to Mentor transition
      topColor
        .setHex(COLORS.WELCOME.TOP)
        .lerp(new THREE.Color(COLORS.MENTOR.TOP), welcomeProgress);
      bottomColor
        .setHex(COLORS.WELCOME.BOTTOM)
        .lerp(new THREE.Color(COLORS.MENTOR.BOTTOM), welcomeProgress);
      ...
      const normalizedProgress =
        mentorProgress > transitionStart
          ? (mentorProgress - transitionStart) / (1 - transitionStart)
          : 0;

      topColor
        .setHex(COLORS.MENTOR.TOP)
        .lerp(new THREE.Color(COLORS.MEET_TIA.TOP), normalizedProgress);
      bottomColor
        .setHex(COLORS.MENTOR.BOTTOM)
        .lerp(new THREE.Color(COLORS.MEET_TIA.BOTTOM), normalizedProgress);
    ...
    try {
      ...
      gradientMaterial.uniforms.offset.value =
        Math.max(
          heroProgress,
          welcomeProgress,
          mentorProgress,
