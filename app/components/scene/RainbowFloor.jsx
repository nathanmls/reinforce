...
// Define rainbow arc models
const RAINBOW_ARCS = [
  // { file: "/models/Arc01.glb", y: -10.2 },
  { file: "/models/Arc02.glb", y: -10.1 },
  { file: "/models/Arc03.glb", y: -10.0 },
  { file: "/models/Arc04.glb", y: -9.9 },
  { file: "/models/Arc05.glb", y: -9.8 },
  { file: "/models/Arc06.glb", y: -9.7 },
  { file: "/models/Arc07.glb", y: -9.6 },
];

// Fallback colors for when models fail to load
...
// Component to display a single arc model
function ArcModel({ file, position, index, isActive, pulseHeight = 2 }) {
  ...
  useEffect(() => {
    if (isActive) {
      ...
      const animationTimeout = setTimeout(() => {
        // Phase 1: Quick rise and scale up
  ...
  // Update position and scale with smooth animation
  useFrame(() => {
    if (arcRef.current) {
      ...
      arcRef.current.scale.x = THREE.MathUtils.lerp(
        arcRef.current.scale.x,
        scale,
        0.1
      );
      arcRef.current.scale.y = THREE.MathUtils.lerp(
        arcRef.current.scale.y,
        scale,
        0.1
      );
      arcRef.current.scale.z = THREE.MathUtils.lerp(
        arcRef.current.scale.z,
        scale,
        0.1
      );
    }
...
export function RainbowFloor() {
  ...
  const [modelsLoaded, setModelsLoaded] = useState(true); // Default to true
  ...
  useEffect(() => {
    ...
    // Find which section has the highest progress between 0.1 and 0.9
    // This indicates the user is viewing that section
    ...
    <>
      {RAINBOW_ARCS.map(({ file, y }, index) => (
        <ArcModel
          ...
          position={[0, y, 0]}
          index={index}
          isActive={shouldPulse || pulseCount > 0}
          pulseHeight={0.3}
        />
      ))}
