...
export const elevenLabsService = {
  ...
  async createAgent(agentData, customApiKey = null) {
    ...
    if (!isBrowser) {
      throw new Error(
        'ElevenLabs API can only be used in a browser environment'
      );
    }
    ...
    try {
      ...
      const requestBody = {
        conversation_config: {
          agent: {
            ...
            first_message:
              agentData.first_message || 'Hello, how can I help you today?',
      ...
      console.log(
        'Creating ElevenLabs agent with data:',
        JSON.stringify(requestBody, null, 2)
      );

      const response = await fetch(
        `${ELEVENLABS_API_URL}/v1/convai/agents/create`,
        {
          method: 'POST',
          headers: {
  ...
  async getAgent(agentId, customApiKey = null) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to get agent: ${errorData.detail || response.statusText}`
        );
      }
      ...
      console.error(
        `Error getting ElevenLabs agent with ID ${agentId}:`,
        error
      );
      throw error;
    }
  ...
  async updateAgent(agentId, agentData, customApiKey = null) {
    ...
    try {
      ...
      console.log(
        'Updating ElevenLabs agent with data:',
        JSON.stringify(requestBody, null, 2)
      );

      const response = await fetch(
        `${ELEVENLABS_API_URL}/v1/convai/agents/${agentId}/edit`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  async deleteAgent(agentId, customApiKey = null) {
    ...
    try {
      const response = await fetch(
        `${ELEVENLABS_API_URL}/v1/convai/agents/${agentId}`,
        {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
      ...
      console.error(
        `Error deleting ElevenLabs agent with ID ${agentId}:`,
        error
      );
      throw error;
    }
